...
- Live verification on 2026-05-31 is blocked because the upstream is unreachable from the migration and Bazarr test networks:
  - `https://www.podnapisi.net/subtitles` does not resolve from the migration network.
  - `https://podnapisi.net/subtitles` does not resolve from the migration network.
  - Fresh recheck after the dead-origin report still returned `curl: (6) Could not resolve host` for both hosts.
  - Bazarr test host recheck also returned `curl: (6) Could not resolve host` for `https://www.podnapisi.net/` and `https://podnapisi.net/`.
  - RDAP still lists `PODNAPISI.NET`, with `last changed` at `2026-05-20T13:24:04Z`. Public web browsing can load the homepage, but the provider remains blocked for Provider Hub migration until the Bazarr test network can prove search and download.
