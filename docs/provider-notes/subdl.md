...
- An empty `unpack_files` list is treated as no member listing, so a season pack falls back to its episode range and season checks.
- Search rows can carry download URLs signed with the caller's `api_key` query parameter (observed live on 2026-09-25). The provider removes the key from every URL it puts in a candidate or payload, records that the URL was signed, and adds the configured key back only when it sends the download request, and only to `https://dl.subdl.com`. Error messages name the unsigned URL.
- Every download URL, signed or not, must be on `subdl.com` or one of its subdomains, with no credentials and the default port. A plain `http` link on those hosts is fetched over HTTPS, which is where `dl.subdl.com` redirects it anyway (checked live on 2026-09-25). Any other URL is rejected before a request is made, and a redirect is followed only to HTTPS on those hosts.
...
- Source selection uses release metadata inside each hearing-impaired class and preserves SubDL's order when sources match equally. A matching subtitle from the same episode suppresses the virtual candidate. A row from another episode, or one that names another season, does not.
