...
- Search rows can carry download URLs signed with the caller's `api_key` query parameter (observed live on 2026-09-25). The provider removes the key from every URL it puts in a candidate or payload, records that the URL was signed, and adds the configured key back only when it sends the download request, and only to `https://dl.subdl.com`. A signed URL on any other origin is fetched without the key. Error messages name the unsigned URL.
