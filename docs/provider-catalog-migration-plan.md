...
- Main-branch catalog inventory: 20 bundles currently ship from `main` after the Gestdown, BSPlayer, Subtis, SubtitulamosTV, TVSubtitles, GreekSubs, AnimeKalesi, and AnimeSub.info merges.
...
- OpenSubtitles.org current branch evidence: `catalog-opensubtitles` at `6c084ae` uses `ai-cloudscraper==3.8.4`, inline Anubis solving, request throttling, optional FlareSolverr fallback for Cloudflare challenges, and a legacy `cloudscraper` argument retry for runtimes that reject `enable_cookie_persistence`. The 2026-06-02 review-fix pass adds native tag lookup URLs, requested-language fetches for direct IMDb listings, page-language fallback for advertised languages such as `hin`, forced and hearing-impaired row flags, real Provider Hub score fields, no synthetic hash matches, wrong-FPS candidate retention with lowered matches, cleaned release names, and episode mismatch rejection for no-IMDb episode searches. PR `#16` is open and head `6c084ae`; current unresolved non-outdated review-thread query returns `5` anchors that remain reviewer-owned, while local regressions cover the requested behavior.
...
- Current catalog checkout inventory: this planning worktree is intentionally not rebased onto `main`, but live `main` now ships 20 Provider Hub bundles, adding `gestdown`, `bsplayer`, `subtis`, `subtitulamostv`, `tvsubtitles`, `greeksubs`, `animekalesi`, and `animesubinfo` to the previous 12 bundle baseline.
...
- OpenSubtitles.org current state: branch `catalog-opensubtitles` is clean at `6c084ae` and PR `#16` is open. Local review-fix validation on 2026-06-02 passed `test_opensubtitles_org.py` with `15` tests, `test_catalog.py` with `12` tests and `6` skipped, `sdk validate`, `py_compile`, full `unittest discover -s tests` with `343` tests and `6` skipped, `git diff --check`, and attribution or prohibited punctuation scans. Bazarr compat proof is still incomplete because the compat key cannot stage or enable Provider Hub bundles and the current compat search results do not include `opensubtitles_org`.
...
- Existing provider worktrees:
  ...
  - `opensubtitles`: branch `catalog-opensubtitles`, worktree `/tmp/bazarr_catalog_provider_worktrees/opensubtitles`, current head `6c084ae`
...
PR `#16` and commit `6c084ae` have local validation and direct native search/download proof. Fresh evidence on 2026-06-02: `test_opensubtitles_org.py` ran `15` tests passed, `test_catalog.py` ran `12` tests passed with `6` skipped, `sdk validate` returned `catalog ok`, `py_compile` passed, full `python3 -B -m unittest discover -s tests` ran `343` tests passed with `6` skipped, `git diff --check` was clean, and attribution or prohibited punctuation scans found no matches. Fresh PR evidence on 2026-06-02: PR `#16` head is `6c084aefe8e91751765752dfe4483581a98126a6`; GitHub reported merge state `UNKNOWN` immediately after push while recalculating; current unresolved non-outdated review-thread query returned `5` anchors. Those anchors cover behavior now pinned by regressions: score fields, advertised language fallback, tag lookup, requested-language direct IMDb fetch, and no-IMDb episode narrowing. The remaining gates are reviewer resolution of those anchors and Bazarr compat proof after an admin-enabled Provider Hub install or update on `bazarr-ui-test`.
