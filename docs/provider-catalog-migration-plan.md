...
- Addic7ed compat clearance on 2026-06-03: PR `#56` is open, ready for review, merge state `CLEAN`, and head `1b54de453e18011eb14cf4bf9d15b6859b7ee527`. Live GitHub shows `17` remaining draft PRs. Addic7ed is SDK and disposable Provider Hub compat-cleared with fresh cookies as direct trusted built-in replacement id `addic7ed`; the four Codex review threads are outdated after the review-fix commit. Titulky remains gated by a non-VIP account, OpenSubtitles.com remains gated by a missing real API key, YavkaNet is search-cleared with official FlareSolverr but gated by source-side direct download HTTP `403`, TurkceAltyazi.org is challenge-cleared with official FlareSolverr but gated by Cloudflare `522` origin timeout pages, and Shooter remains gated by Provider Hub compat proof requiring either approval to disclose a real derived hash or a non-sensitive library-backed fixture.
- Addic7ed follow-up merge on 2026-06-03: PR `#56` was updated to `874fef0bc13bd25afef6a5374272753926aee1fd`, fixing the remaining parser review feedback for multi-language movie tables, `epodd` episode rows, and the current `srch.php?Submit=Search&search=...` movie lookup route. Focused Addic7ed tests, catalog tests, `sdk validate`, `py_compile`, and `git diff --check` passed. Live Addic7ed SDK smoke could not be repeated because direct probes to `https://www.addic7ed.com/`, `/shows.php`, and `/srch.php` timed out from this machine, but the PR review threads were all outdated and PR `#56` merged at `880ec59da3f54d4bc58e9b81a2dfc633328325c2`.
- Hash-gated provider fallback on 2026-06-03: Napisy24 PR `#61` head `b502dd3` now computes the OpenSubtitles-style Napisy24 hash from an existing local video path when `video["hashes"]["napisy24"]` is missing, and Shooter PR `#71` head `58d05f8` now computes Shooter's four-part file hash from an existing local video path when `video["hashes"]["shooter"]` is missing. Focused provider tests, catalog tests, `sdk validate`, `py_compile`, and `git diff --check` passed for both branches. Live upstream proof still requires a real local media file whose computed hash is known by the respective upstream database.
- TurkceAltyazi.org recheck on 2026-06-03: direct `https://turkcealtyazi.org/` returned Cloudflare HTTP `403` with `cf-mitigated: challenge`. Official FlareSolverr `v3.5.0` solved the challenge in about `29` seconds, but SDK smoke with `flaresolverr_timeout_ms=60000` still failed at the access check with Cloudflare HTTP `522`, so the current blocker remains upstream origin timeout after challenge clearance.
- Wizdom origin recheck on 2026-06-03: direct probes to `https://wizdom.xyz/`, `http://wizdom.xyz/`, and `https://www.wizdom.xyz/` all returned Cloudflare HTTP `522`. Treat Wizdom as potential dead-origin and recheck later before spending more migration or compat-validation time.
- YavkaNet download notice on 2026-06-03: keep YavkaNet marked as per-title source-side availability limited, not provider-wide dead. The June 2 site banner says some titles are no longer available for downloads and recommends `bultor.net` or `nanoset.biz`; a June 3 recheck of `https://yavka.net/subs/32811/BG` through official FlareSolverr v3.5.0 returned a current direct `/download?q=...` link and extracted a non-empty SRT.
...
- Post-SDK-fix origin-gate recheck on 2026-06-02: fresh live smoke still shows Wizdom timing out against `wizdom.xyz`. Later same-day solver refreshes moved YavkaNet to search-cleared but download HTTP `403`, and moved TurkceAltyazi.org to challenge-cleared but Cloudflare `522` after clearance.
- PR body hygiene and blocker wording pass on 2026-06-02: live PR descriptions for OpenSubtitles.com `#50` and TurkceAltyazi.org `#53` were cleaned to remove prohibited attribution/vendor scan text, while YavkaNet `#42`, TurkceAltyazi.org `#53`, and Wizdom `#72` now state the tested `ai-cloudscraper`, inline Anubis, FlareSolverr, Cloudflare, and origin-timeout evidence more precisely. A full scan of all `18` open PR bodies found no prohibited attribution/vendor terms and no em dashes; the edited PRs remain draft and merge state `CLEAN`.
- TurkceAltyazi.org official FlareSolverr refresh on 2026-06-02: PR `#53` head `e642b21` raises FlareSolverr default and cap to `60000` ms, keeps the general Cloudflare bypass rule explicit, `ai-cloudscraper` first, inline Anubis second, optional FlareSolverr third, and changes the access check to the normal `30` second HTTP timeout. Official FlareSolverr `v3.5.0` solves the challenge in about `29200` to `33400` ms with `60000` ms, but both provider smoke and direct search probes then return Cloudflare `522` origin timeout pages.
- Community-validation wording pass on 2026-06-02: PR `#50`, `#56`, `#65`, `#61`, `#71`, `#55`, `#27`, and `#58` now explicitly ask for the missing real API key, fresh cookies or login flow, VIP account, hash-bearing fixture, non-sensitive Shooter fixture or approval, Whisper endpoint, Subsarr base URL, or Ktuvit credentials needed to finish live SDK and Provider Hub compat proof. Follow-up scans over all `18` open PR bodies found no prohibited attribution/vendor terms, no stale attribution-scan wording, and no em dashes; the edited PRs remain draft and merge state `CLEAN`.
- Fresh normalized coverage and main validation on 2026-06-02: current Bazarr source still has `60` provider-class modules after helper exclusions, there are `60` matching provider worktrees, current `main` ships `50` provider bundles, and live GitHub has `18` open provider PRs. The normalized missing-worktree and extra-worktree comparisons returned no rows. The only legacy modules not on `main` or an active provider PR are the four dead-origin notes-only providers: `hosszupuska`, `podnapisi`, `subscenter`, and `xsubs`. Current `main` at `2b6035bd31030ad8039b489ee19e41b744e9a5af` passed `test_catalog.py` with `16` tests and `6` skipped, `sdk validate`, `py_compile sdk/cli.py`, `git diff --check HEAD`, and full `unittest discover -s tests` with `851` tests and `6` skipped.
...
- Open PR gate classification on 2026-06-02 continuation: live GitHub state now shows `18` open provider PRs, all draft. No non-draft provider PRs remain open after merging the previously cleared local compat-proof queue.
  ...
  - SDK-cleared but Provider Hub compat gated: none after disposable Provider Hub compat proof for direct replacement `addic7ed`.
  ...
  - Compat-only gated: none after local disposable Provider Hub compat proof for the previously gated providers.
  ...
  - The same `bazarr-compat-remaining` disposable compat run cleared Zimuku's compat gate after switching the official source to `catalog-zimuku`, activating Zimuku `0.1.3` at commit `6fab89144af39b6deda24c0921751500d354a0d6`, installing hash-checked dependencies including `py7zz==1.1.4`, and proving compat search/download/stream with HTTP `200`.
  - Integrated ready provider PRs into `main` on 2026-06-02: BetaSeries `#29` at merge `8feb711acfab74a25354f9b30ef3bdae38e885a3`, Jimaku `#45` at `a5accb2f44b2bc1d9418619b91b2e68c804c726c`, SubDL `#46` at `e3db7fdb7ec724bc525f2647da5c005d8112cab5`, SubSource `#47` at `1d5131c18f8439243a7204b7217731adde96e523`, Subs.ro `#48` at `0c554fab296d990d4ed1cbfcd059ac1bfbfb05b9`, SubX `#49` at `af147d812343600e26176f25d74a458bbb638b4e`, Legendas.net `#60` at `108834c57e9111181220f0f536aed9938c177928`, Titlovi `#64` at `6630d9636c5b40c54971a7d8463f664369634296`, Zimuku `#69` at `569d4c087da6f701bbb801a0b53d77cf0458b39d`, and RegieLive `#70` at `2f943bdd7fab051ea850d1345d6aca1a3e876227`.
  - Current remote compat reachability evidence still needs refresh if `bazarr-ui-test` proof is required instead of local disposable proof: `bazarr-ui-test` does not resolve over SSH, and `BAZARR_COMPAT_API_KEY` is unset in this shell.
...
- Current catalog checkout inventory: this planning worktree is intentionally not rebased onto `main`, but live `main` now ships 50 Provider Hub bundles after adding BetaSeries, Jimaku, SubDL, SubSource, Subs.ro, SubX, Legendas.net, Titlovi, Zimuku, and RegieLive to the previous 40 bundle baseline.
...
- Existing provider worktrees:
  - `addic7ed`: branch `catalog-addic7ed`, worktree `/tmp/bazarr_catalog_provider_worktrees/addic7ed`, current head `1b54de4`
  ...
  - `turkcealtyaziorg`: branch `catalog-turkcealtyaziorg`, worktree `/tmp/bazarr_catalog_provider_worktrees/turkcealtyaziorg`, current head `e642b21`
...
- Current checkpoint: `1b54de4 Address Addic7ed review feedback`
...
- Fresh cookie recheck on 2026-06-02:
  ...
  - `python3 -B -m sdk validate`: `catalog ok`.
  - `git diff --check origin/main...HEAD`: clean.
  - Initial live SDK smoke with an incomplete saved temporary Addic7ed cookie reached the cookie check but failed with `Addic7ed cookies are not valid anymore`.
  - After updating the temporary cookie value with `wikisubtitlesuser` and `wikisubtitlespass`, `python3 -B -m sdk smoke-test --provider addic7ed --language eng --video-fixture tests/fixtures/subs4series_video_game_of_thrones_s01e01.json --config-json <saved cookies and user agent> --expect-min-results 1 --skip-download` returned `addic7ed ok`.
  - Full SDK live smoke with the same saved cookies and User-Agent, without `--skip-download`, returned `addic7ed ok` after one retry for an upstream read timeout.
...
- Provider Hub ID and compat proof on 2026-06-03:
  ...
  - The earlier non-shadowing `addic7ed_com` experiment proved the provider logic but was not the final migration shape, because Addic7ed is intended to be a trusted built-in replacement.
  - Commit `d66da6bebde6cc74b85d9e3ca985632c14124248` restores catalog id `addic7ed`.
  - `python3 -B -m pytest tests/test_addic7ed.py tests/test_catalog.py -q`: `13` passed, `6` skipped.
  - `python3 -B -m sdk validate`: `catalog ok`.
  - `python3 -B -m py_compile providers/addic7ed/provider.py`: passed.
  - `git diff --check`: clean.
  - Live SDK smoke with saved cookies and User-Agent returned `addic7ed ok`.
  - Disposable Bazarr compat container `bazarr-compat-remaining` installed and activated `addic7ed` at commit `d66da6bebde6cc74b85d9e3ca985632c14124248`, `state: active`, `pending_restart: false`, using the trusted built-in replacement policy from the core replacement branch.
  - A stale disposable `addic7ed_com` install was disabled before the final compat proof, because its duplicate results hid the direct replacement rows in the reusable test config.
  - Compat `/api/v1/subtitles` returned HTTP `200`, `total_count: 34`, and `addic7ed_count: 9`.
  ...
  - Review-fix commit `1b54de453e18011eb14cf4bf9d15b6859b7ee527` addresses the four unresolved Codex review threads: Brazilian Portuguese regional matching, duplicate `Set-Cookie` preservation, partial movie translation rejection, and multi-episode search matching.
  - Red tests for those four review items failed before implementation, then `python3 -B -m pytest tests/test_addic7ed.py -q` passed with `9` tests.
  - `python3 -B -m pytest tests/test_addic7ed.py tests/test_catalog.py -q`: `17` tests passed, `6` skipped.
  - `python3 -B -m sdk validate`: `catalog ok`.
  - `python3 -B -m py_compile providers/addic7ed/provider.py`: passed.
  - `git diff --check`: clean.
  - Live SDK smoke with saved cookies and User-Agent returned `addic7ed ok`.
  - Disposable replacement-policy compat proof on commit `1b54de453e18011eb14cf4bf9d15b6859b7ee527` returned `/api/v1/subtitles` HTTP `200`, `total_count: 34`, `addic7ed_count: 9`, `/api/v1/download` HTTP `200`, and stream HTTP `200` with `38750` bytes, `application/x-subrip`.
  - Live GraphQL review-thread recheck shows the four Addic7ed review threads are now `isOutdated: true`; no non-outdated unresolved review thread remains.
  - Attempted merge was blocked by local safety policy because this continuation turn did not include explicit approval to mutate the default branch. PR `#56` remains open, ready for review, and clean.
- Remaining gates:
  ...
  - None for cookie-backed search/download/compat proof.
...
- Provider Hub compat fix and proof on 2026-06-02:
  ...
  - Follow-up SDK tooling PR `#78`, branch `sdk-smoke-regional-language` at `c676f7b`, fixes the smoke-test harness so the CLI form `--language por-BR` accepts the provider's normalized `alpha3=por`, `country_alpha2=BR` language payload. The live search-only CLI smoke returned `legendasnet ok` with the saved temporary credential, and the PR merged into `main` at `2b6035bd31030ad8039b489ee19e41b744e9a5af`.
...
- Provider Hub compat proof on 2026-06-02:
  - Local disposable Provider Hub proof used `ghcr.io/lavx/bazarr:ui-test` container `bazarr-compat-remaining` on `127.0.0.1:6770`; Provider Hub reported `state=active`, `pending_restart=false`, and active commit `6fab89144af39b6deda24c0921751500d354a0d6`.
  - Compat search `GET /api/v1/subtitles?imdb_id=0944947&query=Game.of.Thrones.S01E01.HDTV.XviD-FEVER&type=episode&season_number=1&episode_number=1&languages=zh&per_page=100` returned HTTP `200`, `5` total results, and `5` Zimuku rows.
...
- Provider Hub compat proof on 2026-06-02:
  - Local disposable Provider Hub proof used `ghcr.io/lavx/bazarr:ui-test` container `bazarr-compat-remaining` on `127.0.0.1:6770`; Provider Hub reported `state=active`, `pending_restart=false`, and active commit `8e228c910cebcd074150696bbc02f5abc9c76b21`.
  - Compat search `GET /api/v1/subtitles?imdb_id=0903747&query=Breaking.Bad.S03E13.720p.HDTV.x264-REWARD&type=episode&season_number=3&episode_number=13&languages=fr&per_page=100` returned HTTP `200`, `9` total results, and `9` BetaSeries rows.
  - Compat download `POST /api/v1/download` for first BetaSeries `file_id=1` returned HTTP `200`.
  - Compat stream returned HTTP `200`, `30175` bytes, and content type `application/x-subrip`.
- Remaining gates: none for local disposable Provider Hub compat proof.
...
- Local FlareSolverr comparison on 2026-06-02:
  ...
  - Temporary local FlareSolverr container was removed after testing.
- Current-main refresh on 2026-06-02:
...
- Direct working-title recheck on 2026-06-03:
  - Direct curl from this machine to `https://yavka.net/subs/32811/BG` still returned HTTP `403` with `cf-mitigated: challenge`, so plain HTTP remains Cloudflare-blocked.
  - Official `ghcr.io/flaresolverr/flaresolverr:v3.5.0` on `127.0.0.1:8191` solved the same URL in 17.45 seconds, returned status `ok`, solution status `200`, response length `136373`, and cookies including `cf_clearance`.
  - The solved page exposed `id="down"` with a current direct `/download?q=...` link, matching the YavkaNet parser.
  - One-off provider smoke using the same FlareSolverr session parsed the detail page, fetched `/download`, extracted an SRT payload, and returned format `srt`, content type `application/x-subrip`, size `174220` bytes, and SHA-256 `0f6ac19c15adfa5e65c89c84fe145e52f6e021ae15768698a97fb3947260dd26`.
- Remaining gates:
  - Treat current YavkaNet proof as search-cleared and direct-download capable for at least some titles through official FlareSolverr `v3.5.0`, with per-title source availability limits still expected.
  - Prove Provider Hub compat search, download, and stream on `bazarr-ui-test` with a title that currently permits subtitle downloads.
...
- Current-main merge evidence on 2026-06-02:
  ...
  - Fresh SDK recheck on 2026-06-02 using the temporary Jimaku key from `/tmp/bazarr_provider_test_credentials.env`: `python3 -B -m unittest discover -s tests -p 'test_jimaku.py'` ran `15` tests passed, `python3 -B -m unittest discover -s tests -p 'test_catalog.py'` ran `14` tests passed with `6` skipped, `python3 -B -m sdk validate` returned `catalog ok`, and `git diff --check origin/main...HEAD` was clean.
  - `python3 -B -m sdk smoke-test --provider jimaku --language jpn --video-fixture tests/fixtures/jimaku_video_frieren_s01e05.json --config-json '{"enable_name_search_fallback":true,"enable_archives_download":false,"enable_ai_subs":false}' --secret api_key=JIMAKU_API_KEY --expect-min-results 1 --skip-download`: `jimaku ok`.
  - `python3 -B -m sdk smoke-test --provider jimaku --language jpn --video-fixture tests/fixtures/jimaku_video_frieren_s01e05.json --config-json '{"enable_name_search_fallback":true,"enable_archives_download":false,"enable_ai_subs":false}' --secret api_key=JIMAKU_API_KEY --expect-min-results 1`: `jimaku ok`.
  - `gh pr view 45 --repo LavX/bazarr-provider-catalog --json number,state,isDraft,mergeStateStatus,reviewDecision,headRefOid,url`: PR `#45` is open, draft, head `d4a1600baf1046e4238c1f85415fb4336acb0c38`, merge state `CLEAN`.
- Provider Hub compat proof on 2026-06-02:
  - Local disposable Provider Hub proof used `ghcr.io/lavx/bazarr:ui-test` container `bazarr-compat-remaining` on `127.0.0.1:6770`; Provider Hub reported `state=active`, `pending_restart=false`, and active commit `d4a1600baf1046e4238c1f85415fb4336acb0c38`.
  - Compat search `GET /api/v1/subtitles?query=Frieren.Beyond.Journeys.End.S01E05.1080p.WEB.H264&type=episode&season_number=1&episode_number=5&languages=ja&per_page=100` returned HTTP `200`, `9` total results, and `9` Jimaku rows.
  - Compat download `POST /api/v1/download` for first Jimaku `file_id=1` returned HTTP `200`.
  - Compat stream returned HTTP `200`, `25983` bytes, and content type `application/x-subrip`.
- Remaining gates: none for local disposable Provider Hub compat proof.
...
- Current-main merge evidence on 2026-06-02:
  ...
  - Fresh SDK recheck on 2026-06-02 using the temporary SubDL API key from `/tmp/bazarr_provider_test_credentials.env`: `python3 -B -m unittest discover -s tests -p 'test_subdl.py'` ran `13` tests passed, `python3 -B -m unittest discover -s tests -p 'test_catalog.py'` ran `14` tests passed with `6` skipped, `python3 -B -m sdk validate` returned `catalog ok`, and `git diff --check origin/main...HEAD` was clean.
...
- Current-main merge evidence on 2026-06-02:
  ...
  - Fresh SDK recheck on 2026-06-02 using the temporary SubSource API key from `/tmp/bazarr_provider_test_credentials.env`: `python3 -B -m unittest discover -s tests -p 'test_subsource.py'` ran `10` tests passed, `python3 -B -m unittest discover -s tests -p 'test_catalog.py'` ran `14` tests passed with `6` skipped, `python3 -B -m sdk validate` returned `catalog ok`, and `git diff --check origin/main...HEAD` was clean.
...
- Current-main merge evidence on 2026-06-02:
  ...
  - Fresh SDK recheck on 2026-06-02 using the temporary SubX key from `/tmp/bazarr_provider_test_credentials.env`: `python3 -B -m unittest discover -s tests -p 'test_subx.py'` ran `15` tests passed, `python3 -B -m unittest discover -s tests -p 'test_catalog.py'` ran `14` tests passed with `6` skipped, `python3 -B -m sdk validate` returned `catalog ok`, and `git diff --check origin/main...HEAD` was clean.
  - Raw live SubX API probes outside the sandbox returned `5` items for Inception title search, Inception IMDb search, Breaking Bad episode title search, and title-only Inception search. The sandbox DNS failure path returns empty provider results after retries, so live SubX proof must run outside the sandbox.
  - `python3 -B -m sdk smoke-test --provider subx --language spa --video-fixture tests/fixtures/subclub_video_inception.json --config-json '{"request_delay_ms":0}' --secret api_key=SUBX_API_KEY --expect-min-results 1 --skip-download`: `subx ok`.
...
- Current checkpoint: `e642b21 Refresh TurkceAltyazi Cloudflare fallback`
- PR: `https://github.com/LavX/bazarr-provider-catalog/pull/53` opened as draft on 2026-06-01, head `catalog-turkcealtyaziorg`, base `main`, head `e642b212d8bf0fc00c55ae177bfe736b69268b4b`, latest checked merge state `CLEAN`.
- Baseline evidence on 2026-05-31:
  - Fresh worktree baseline `python3 -B -m sdk validate`: `catalog ok`.
  - Fresh worktree baseline `python3 -B -m unittest discover -s tests`: `328` tests passed, `6` skipped.
- Local evidence on 2026-05-31:
...
- Local FlareSolverr comparison on 2026-06-02:
  ...
  - Temporary local FlareSolverr container was removed after testing.
- Official FlareSolverr timeout refresh on 2026-06-02:
  - Direct local official FlareSolverr `v3.5.0` probes against `https://turkcealtyazi.org` showed `25000` ms and `30000` ms are too low, while `60000` ms solves Cloudflare in about `29200` to `33400` ms.
  - Red TDD gate `python3 -m pytest tests/test_turkcealtyaziorg.py -q`: failed because a configured `45000` ms FlareSolverr timeout was still clamped to `25000` ms.
  - Red TDD gate `python3 -m pytest tests/test_turkcealtyaziorg.py::TurkceAltyaziSearchTests::test_access_check_uses_normal_http_timeout -q`: failed because the access check used a hard-coded `10` second timeout instead of `HTTP_TIMEOUT_SECONDS`.
  - `e642b21` raises the provider FlareSolverr default and cap to `60000` ms, keeps the OpenSubtitles.org-style bypass order documented as `ai-cloudscraper` first, inline Anubis second, optional FlareSolverr third, changes the access check to the normal `30` second HTTP timeout, bumps TurkceAltyazi.org to `0.1.3`, refreshes bundle hashes, and updates PR `#53`.
  - Verification passed: `python3 -m pytest tests/test_turkcealtyaziorg.py -q` with `16` tests, `python3 -m pytest tests/test_turkcealtyaziorg.py tests/test_catalog.py -q` with `25` tests and `6` skipped, full `python3 -m pytest -q` with `860` tests, `6` skipped, and `36` subtests, `python3 -m sdk validate`, `python3 -m sdk runtime-matrix`, `python3 -m py_compile providers/turkcealtyaziorg/provider.py`, `git diff --check`, and touched-file prohibited text scan.
  - Provider live smoke with official FlareSolverr `v3.5.0`, `flaresolverr_url=http://127.0.0.1:8191/v1`, and `flaresolverr_timeout_ms=60000` reached the solved path, then failed with `TurkceAltyazi access check failed with HTTP 522`.
  - Direct FlareSolverr search probe for `https://turkcealtyazi.org/find.php?cat=sub&find=1375666` solved Cloudflare, but the returned page was Cloudflare `522: Connection timed out`, so the remaining blocker is current origin timeout after challenge clearance.
- Remaining gates:
  - Re-run SDK live smoke search and download when TurkceAltyazi.org stops returning Cloudflare `522` origin timeout pages after challenge clearance, or with valid TurkceAltyazi.org cookies and User-Agent that bypass the current gate.
  ...
  - Prove Provider Hub compat search, download, and stream on `bazarr-ui-test` after the origin returns normal search pages instead of Cloudflare `522`, or with working cookies paired with the same browser User-Agent.
