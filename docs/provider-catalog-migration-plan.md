...
- Main-branch catalog inventory: 21 bundles currently ship from `main` after the Gestdown, BSPlayer, Subtis, SubtitulamosTV, TVSubtitles, GreekSubs, AnimeKalesi, AnimeSub.info, and OpenSubtitles.org merges.
...
- Current catalog checkout inventory: this planning worktree is intentionally not rebased onto `main`, but live `main` now ships 21 Provider Hub bundles, adding `gestdown`, `bsplayer`, `subtis`, `subtitulamostv`, `tvsubtitles`, `greeksubs`, `animekalesi`, `animesubinfo`, and `opensubtitles_org` to the previous 12 bundle baseline.
