...
- Open PR gate classification on 2026-06-02 continuation: live GitHub state still shows `28` open provider PRs, all draft, all merge state `CLEAN`.
  ...
  - Compat-only gated: `regielive`, `titlovi`, and `zimuku` have current SDK search and download proof, but still need Provider Hub compat search, download, and stream proof.
