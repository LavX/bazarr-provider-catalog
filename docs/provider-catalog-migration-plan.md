...
- SDK regional smoke validation on 2026-06-02: catalog PR [#78](https://github.com/LavX/bazarr-provider-catalog/pull/78), branch `sdk-smoke-regional-language` at `c676f7b`, adds smoke-test coverage for requested regional codes such as `por-BR` when providers return normalized language payloads such as `alpha3=por` plus `country_alpha2=BR`. Verification passed `test_catalog.py`, `sdk validate`, `py_compile sdk/cli.py`, full `unittest discover -s tests` with `851` tests and `6` skipped, `git diff --check`, prohibited-text scan, and live Legendas.net `sdk smoke-test --provider legendasnet --language por-BR ... --skip-download` with `legendasnet ok`.
...
- Provider Hub compat fix and proof on 2026-06-02:
  ...
  - Follow-up SDK tooling PR `#78`, branch `sdk-smoke-regional-language` at `c676f7b`, fixes the smoke-test harness so the CLI form `--language por-BR` accepts the provider's normalized `alpha3=por`, `country_alpha2=BR` language payload. The live search-only CLI smoke returned `legendasnet ok` with the saved temporary credential.
