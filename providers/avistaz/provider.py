"""AvistaZ provider for the Bazarr+ Provider Hub catalog."""
...
class AvistazProvider:
    ...
    def search(self, video, languages, config):
        ...
        if not _is_avistaz_url(info_url):
            return []
        ...
        if response.status == 404:
            return []
        _raise_for_status(response, "AvistaZ release page")
    ...
    def _ensure_cookies(self, cookies, config):
        ...
        if response.status in {302, 403, 404}:
            raise PermissionError("AvistaZ cookies are not valid anymore")
        _raise_for_status(response, "AvistaZ cookie validation")
...
def _parse_cookies(config):
    ...
    if not value:
        raise ValueError("AvistaZ cookies are required")
    cookie = SimpleCookie()
    cookie.load(value)
...
def parse_release_page(body, page_url):
    ...
    if title_cell is None:
        raise RuntimeError("AvistaZ release page did not include a title row")
